...
public class MinHeapD implements HeapTDA {
    class Nodo {
        int valor;
        Nodo ant;
        Nodo sig;
    ...
    public void insertar(int valor) {
        ...
        heapifyUp(); // Se asegura de que el nodo se mueva hacia arriba hasta que sea menor que su padre
    }
    ...
    public void quitar() {
        ...
        heapifyDown(); // Se ordena para que se restaure la propiedad de min-heap, asegurando que el nodo sea más pequeño que sus hijos.
    ...
    private void heapifyUp() { // Se asegura de que el nodo se mueva hacia arriba hasta que sea menor que su padre
        int indice = cant;
        ...
        while (indice > 1) { // Se recorre el heap hacia arriba desde el nodo recién insertado
            ...
            if (nodo.valor < padre.valor) { // Se entra al if si el valor del hijo es menor al del padre y se intercambian
                int aux = nodo.valor;
                nodo.valor = padre.valor;
                padre.valor = aux;
                indice = padreInd; // El nodo ahora está en la posición del padre, seguimos comparando hacia arriba
            } else {
                break; // Si el nodo ya no es menor que su padre, el heap ya está ordenado
            }
        }
    }

    private void heapifyDown() { // Se ordena para que se restaure la propiedad de min-heap, asegurando que el nodo sea más pequeño que sus hijos.
        ...
        while (indice <= cant / 2) {  // Mientras el nodo tenga hijos
            ...
            if (obtenerNodoPorIndice(indice).valor <= obtenerNodoPorIndice(menor).valor) { // Si el valor del nodo es menor o igual que el de su hijo más pequeño, se termina
                break;
            }
            
            Nodo nodo = obtenerNodoPorIndice(indice); // Si no, se intercambia el valor del nodo con el de su hijo más pequeño
            Nodo hijo = obtenerNodoPorIndice(menor);
            
            int aux = nodo.valor;
            nodo.valor = hijo.valor;
            hijo.valor = aux;
            
            indice = menor; // El nuevo nodo a comprobar es el del anteriormente hijo que subió en el árbol binario
        }
    }
